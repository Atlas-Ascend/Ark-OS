...
[ -f "$ARK_ROOT/scripts/resident-edge/odin/emit-odin-identity-receipt.sh" ] || {
  echo "ODIN_WORKFORCE_PROOF=FAIL reason=identity_emitter_missing" >&2
  exit 21
}
...
[ -n "$identity_receipt" ] && [ -f "$identity_receipt" ] || {
  echo "ODIN_WORKFORCE_PROOF=FAIL reason=identity_receipt_missing" >&2
  exit 23
}
...
case "$remote" in
  ...
  *) echo "ODIN_WORKFORCE_PROOF=FAIL reason=unexpected_remote:$remote" >&2; exit 24 ;;
esac
[ "$current_branch" = "$WORKFORCE_BRANCH" ] || {
  echo "ODIN_WORKFORCE_PROOF=FAIL reason=unexpected_branch:$current_branch" >&2
  exit 25
}

python workforce/prove_resident.py \
  --executor ODIN \
  --executor-proof "$identity_receipt" \
  --require-resident-identity

proof="$WORKDIR/proof/workforce-resident-proof.json"
[ -f "$proof" ] || { echo "ODIN_WORKFORCE_PROOF=FAIL reason=workforce_receipt_missing" >&2; exit 26; }
